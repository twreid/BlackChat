...
void create_window_message(WIN_OBJ win, char *result) {
  sprintf(result, "%02d%02d%03d%04d%04d%02d%04d%04d", CMD_WINDOW, win->type,
          win->wid, win->x, win->y, win->z, win->w, win->h);
}
...
void create_first_user(int user_list_type, int from_uid, UR_OBJ user,
                       char *result) {
  int offset = strlen(user->name) + UID_LEN;

  sprintf(result, "%02d%02d%03d%03d%03d%s", CMD_USERLIST, user_list_type,
          from_uid, offset, user->uid, user->name);
}
