...
void fill_queue(SERVER_OBJ *server, SERVER_QUEUE_OBJ *messages) {
  ...
  for (int i = 1; i < 11; i++) {
    ...
    server->clients[i]->user_data->history = (HST_OBJ)malloc(sizeof(struct history_obj));
    server->clients[i]->user_data->im = (HST_OBJ)malloc(sizeof(struct history_obj));
    ...
    snprintf(server->clients[i]->user_data->name, sizeof(server->clients[i]->user_data->name), "User%d", i);
