...
void update_time(int sig) {
  ...
  char *lurk_text = (char *)calloc(16, sizeof(char));
  char *to_client = (char *)calloc(256, sizeof(char));
  ...
  for (int i = 1; i <= 10; i++) {
    ...
    if (bc_server->clients[i]->is_connected) {
      bc_server->clients[i]->seconds_connected =
          time(NULL) - bc_server->clients[i]->time_connected;
      ...
      sprintf(buff, "Bytes sent: %d | Bytes received: %d\nSeconds Connected: "
                    "%ld | Lurking: %s\nUser name: %s",
              bc_server->clients[i]->bytes_from,
              bc_server->clients[i]->bytes_to,
              bc_server->clients[i]->seconds_connected, lurk_text,
              bc_server->clients[i]->user_data->name);
...
int main(int argc, char **argv) {
  ...
  printf("start main");
  int sem = sem_init(&messages_sem, 0, 1);
  ...
  SERVER_QUEUE_OBJ *messages =
      (SERVER_QUEUE_OBJ *)malloc(sizeof(SERVER_QUEUE_OBJ));
...
void handle_messages(SERVER_OBJ *server, SERVER_QUEUE_OBJ *messages) {
  ...
  case CMD_TEXT:
    ...
    case TEXT_MAIN_CHAT: {
      ...
      server->clients[user]->seconds_connected =
          time(NULL) - server->clients[user]->time_connected;
    ...
    case TEXT_STATUS: {
      ...
      if (user >= 1 && user <= 10) {
        ...
        sprintf(to_client, "%s\n%s", server->clients[user]->user_data->name,
                buff);
    ...
    case TEXT_IM: {
      ...
      get_text_from_message(message,
                            server->clients[user]->user_data->im->line);
      // server->clients[user]->user_data->im->from = NULL;
      // server->clients[user]->user_data->im->next = temp;
      // TODO update time

      create_im_message(user, to_user,
                        server->clients[user]->user_data->im->line, buff);
  ...
  case CMD_USERLIST:
    ...
    case USER_LIST_REQUEST: {
      ...
      create_first_user(USER_LIST_CURRENT, user,
                        server->clients[user]->user_data, buff);

      for (int i = 1; i <= 10; i++) {

        if (server->clients[i]->is_connected &&
            server->clients[i]->user_data->uid != user)
    ...
    case USER_LIST_SIGN_OFF:
      // Prefer not to use this and would rather detect a read of 0 bytes from
  ...
  case CMD_LURK: {
    ...
    if (server->clients[user]->user_data->lurk) {
      sprintf(message_to_server, "%s is no longer lurking!",
              server->clients[user]->user_data->name);
      server->clients[user]->user_data->lurk = 0;
    } else {
      sprintf(message_to_server, "%s is Lurking!!!",
              server->clients[user]->user_data->name);
...
int save_user_window(WIN_OBJ window, int user) {
  ...
  FILE *file;
  int written;
  int win[] = {window->h, window->w,    window->x,  window->y,
               window->z, window->type, window->wid};
  sprintf(file_to_open, "./saved_sessions/%d/%d/%d", user, window->type,
          window->wid);
