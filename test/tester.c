...
char *users[] = {"timothy", "josh",  "henry", "tyler", "david",
                 "mary",    "nancy", "tanya", "austin"};
...
int main() {
  ...
  //	create_first_user(int user_list_type, int from_uid, UR_OBJ user, char
  //*result);
...
void process_command(char *input, char *output) {
  ...
  case CMD_TEXT:
    ...
    if (type == TEXT_IM) {
      ...
      sprintf(output, "cmd:%s(%d) text:%d uid:%d fuid:%d content:%s",
              cmd_type[type_result], type_result, type, user, from_user,
              message);
    } else {
      sprintf(output, "cmd:%s(%d) text:%d uid:%d content:%s",
              cmd_type[type_result], type_result, type, user, message);
    }
  ...
  case CMD_WINDOW:
    ...
    sprintf(output, "cmd:%s(%d) type:%d wid:%d x:%d y:%d z:%d w:%d h:%d",
            cmd_type[type_result], type_result, window->type, window->wid,
            window->x, window->y, window->z, window->w, window->h);
  ...
  case CMD_VOTE:
    ...
    sprintf(output, "cmd:%s(%d) type:%d uid:%d vuid:%d", cmd_type[type_result],
            type_result, type, user, from_user);
    break;
  case CMD_USERLIST:
    ...
    if (type != USER_LIST_REQUEST) {
      char temp_buff[1024];
      sprintf(output, "cmd:%s(%d) type:%d uid:%d", cmd_type[type_result],
              type_result, type, user);
      ...
      do {
        user_num++;
        user_list[user_num] = (UR_OBJ)malloc(sizeof(struct user_obj));
      } while ((offset = get_next_user(offset, input, user_list[user_num])) >
               0);
      ...
      for (i = 0; i < user_num; i++) {
        ...
        sprintf(temp_buff, " offset:%d uid:%d name:%s",
                (int)strlen(curr_user->name) + 3, curr_user->uid,
                curr_user->name);
        strcat(output, temp_buff);

        free(curr_user); // we can get rid of the user since they aren't needed
                         // anymore
      }
  ...
  case CMD_ERROR:
    ...
    sprintf(output, "cmd:%s(%d) type:%d", cmd_type[type_result], type_result,
            type);
    break;
  default:
