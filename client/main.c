...
void print_client_chat_buffer() {
  ...
  if ((strlen(client_buffer) - MAX_CHARS_IN_CLIENT_TYPING_WINDOW) > 0) {
    int num_chars_off_screen =
        strlen(client_buffer) - MAX_CHARS_IN_CLIENT_TYPING_WINDOW;
    ...
    wprintw(client_chat_window,
            &client_buffer[client_current_line * MAX_COLUMNS]);
  } else {
...
int main(int argc, char *argv[]) {
  ...
  while (is_running) {
    ...
    case KEY_DOWN:
      client_current_line++;
      if (client_current_line * MAX_COLUMNS > strlen(client_buffer))
        client_current_line--;
    ...
    /* If were here, that means we didn't press any "special" keys so that means
     * were
