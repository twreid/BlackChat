...
#define MAX_wchar_tS_IN_TRANSCRIPT_WINDOW                                      \
...
void window_page_down(WINDOW *win, int *line, int max_columns,
                      wchar_t *buffer) {
  int i = 0;
 ...
 * max_wchar_ts	 ~ The max number of wchar_tacters that can appear in the
 * windows view at any given time.
 * max_columns   ~ The max number of wchar_tacters that can be in a given
 * column.
 * buffer	 ~ The wchar_tacter buffer to write to the window.
 * curr_line	 ~ The current line the user is at in the window.
 */
static void print_buffer_to_window(WINDOW *win, int max_wchar_ts,
                                   int max_columns, wchar_t *buffer,
                                   int *curr_line) {
  ...
  /* Figure out how many bold wchar_ts are above the current line where
   * printing. */
...
static void print_transcript_chat_buffer() {
  print_buffer_to_window(
      transcript_window, TRANSCRIPT_MAX_ROWS * TRANSCRIPT_MAX_COLUMNS,
      TRANSCRIPT_MAX_COLUMNS, transcript_buffer, &transcript_current_line);

  print_buffer_to_window(fullscreen_transcript_window,
                         TRANSCRIPT_MAX_ROWS * (TRANSCRIPT_MAX_COLUMNS * 2),
                         TRANSCRIPT_MAX_COLUMNS * 2, f_transcript_buffer,
...
static void init_other_windows() {
  ...
  for (i = 0; i < 10; i++) {
    ...
    memset(other_chat_windows[i].userName, '\0',
           sizeof(other_chat_windows[i].userName));
    memset(other_chat_windows[i].buffer, '\0',
           sizeof(other_chat_windows[i].buffer));
...
static void refresh_other_windows() {
  ...
  for (i = 0; i < 10; i++) {
    ...
    /* Make sure we only print the end of the buffer AND/OR print white space so
     * the
    ...
    mvwprintw(other_chat_windows[i].window, 1, 0, textToPrint);
...
/* Delete the specified number of wchar_tacters behind the current cursor
 * position. */
#if 0
...
void set_yell_message(int index, wchar_t *message) {
  int i;
  if (index < 0 || index > 25) {
    log_writeln("WARNING: Can't set yell message because index is not between "
                "0 and 25!");
    return;
  }
  if (wcslen(message) >= MAX_MESSAGE_LENGTH) {
    log_writeln("WARNING: Can't set yell message because message length is "
                "greater than MAX_MESSAGE_LENGTH!");
    return;
  }
...
wchar_t *get_client_name() { return L"Henry"; }
...
void clear_user_window_text(int num) {
  memset(other_chat_windows[num].buffer, '\0',
         sizeof(other_chat_windows[num].buffer));
}
...
void remove_user_from_window(int num) {
  if (num < 0 || num > 9) {
    log_writeln("WARNING: Cannot remove user at index[%d] because it is not a "
                "valid index.  (valid index are 0-9).",
                num);
    return;
  }

  wclear(other_chat_windows[num].window);
  memset(other_chat_windows[num].userName, '\0',
         sizeof(other_chat_windows[num].userName));
  memset(other_chat_windows[num].buffer, '\0',
         sizeof(other_chat_windows[num].buffer));
}

/* Get the text inside our chat window. */
wchar_t *grab_text_from_client_typing_window(void) { return client_buffer; }
...
static wchar_t *write_to_ts_win(wchar_t *str, int max_col, int max_row,
                                WINDOW *win, wchar_t *buffer, int *line,
                                int *buffer_size) {
  ...
  while ((wcslen(buffer) + wcslen(text)) > *buffer_size) {
    int i = 0;
    wchar_t *temp_buffer =
        (wchar_t *)malloc(sizeof(wchar_t) * (*buffer_size * 2));
...
void write_to_transcript_window(wchar_t *str) {
  ...
  for (i = 0; i <= length; i++) {
    ...
    if (str[i] == '\n' || str[i] == '\0') {
      ...
      transcript_buffer = write_to_ts_win(
          s, TRANSCRIPT_MAX_COLUMNS, TRANSCRIPT_MAX_ROWS, transcript_window,
          transcript_buffer, &transcript_current_line, &transcript_buffer_size);
      f_transcript_buffer =
          write_to_ts_win(s, TRANSCRIPT_MAX_COLUMNS * 2, TRANSCRIPT_MAX_ROWS,
                          fullscreen_transcript_window, f_transcript_buffer,
                          &transcript_current_line, &f_transcript_buffer_size);
...
int main(int argc, char *argv[]) {
  ...
  /* josh-note:
                  These need to be uncommented for the client connection to
     work. */
  /*	int client_id = init_client("Henry");                   create a client.
   */
  /*      init_user_list(client_id);                      init the client size
   * user list. */
  ...
  transcript_buffer =
      (wchar_t *)malloc(sizeof(wchar_t) * transcript_buffer_size);
  f_transcript_buffer =
      (wchar_t *)malloc(sizeof(wchar_t) * f_transcript_buffer_size);
  memset(client_buffer, '\0', sizeof(client_buffer));
  memset(transcript_buffer, '\0', sizeof(transcript_buffer));

  get_terminal_size(&x_terminal_size, &y_terminal_size);
  log_writeln(" > ... detecting current terminal size xy:(%d,%d)",
              x_terminal_size, y_terminal_size);
  ...
  fullscreen_transcript_window =
      newwin(TRANSCRIPT_MAX_ROWS, TRANSCRIPT_MAX_COLUMNS * 2, 0, 0);
  ...
  */ // end get wchar_t
    ...
    /* Check if were in IM mode. */
      ...
      /*
       TODO: Display list of users (like deepsix) to send IM to.
              All IM's will be displayed on the main transcript with some type
       of "marker"
      ...
      /* exit IM */
      sending_im = 0;
      window_page_up(transcript_window, &transcript_current_line,
                     TRANSCRIPT_MAX_COLUMNS, transcript_buffer);
      window_page_up(fullscreen_transcript_window, &transcript_current_line,
                     TRANSCRIPT_MAX_COLUMNS * 2, f_transcript_buffer);
    }
    /* Check if were in deepsix mode. */
      ...
      if (ch >= 48 && ch <= 57) {
        /* josh-note:
                ...
                Also, have josh keep track of who user voted for and display
           message on transcript as to how user voted
    ...
    /* Check if were yelling. */
      ...
      switch (ch) {
        ...
        case 10: /* CTRL-J and CTRL-M */
                 /* UNCOMMENT ME FOR USE WITH SERVER */
          {
            ...
            buf = (wchar_t *)malloc(
                (wcslen(L"[Client Says]: ") + wcslen(client_buffer) + 1) *
                sizeof(wchar_t));
          ...
          /*	write_out(client_id);                    enter key is pressed so
        ...
        case 14: /* CTRL-N */
          ...
          window_page_down(transcript_window, &transcript_current_line,
                           TRANSCRIPT_MAX_COLUMNS, transcript_buffer);

          window_page_down(fullscreen_transcript_window,
                           &transcript_current_line, TRANSCRIPT_MAX_COLUMNS * 2,
                           f_transcript_buffer);
          break;
        case 16: /* CTRL-P */
          alarm(5);
        ...
        case 30: /* CTRL-6 */
          ...
          /*
           * If we encountered an unkown escape wchar_tcter, break out of here
           * so we don't
           * print it. */
          break; /* TODO: Fix me! */
        }
      ...
      /* If were here, that means we didn't press any "special" keys so that
       * means were
      ...
      default:
        ...
        if (!iscntrl(ch)) {
          ...
          if (client_cursor_position != wcslen(client_buffer)) {
            ...
            int i;
            for (i = wcslen(client_buffer) + 1; i > client_cursor_position;
                 i--) {
              client_buffer[i] = client_buffer[i - 1];
            }
          }
