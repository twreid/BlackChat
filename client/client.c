...
#define MAX_CHARS_IN_TRANSCRIPT_WINDOW                                         \
 ...
 * max_chars	 ~ The max number of characters that can appear in the windows
 ...
 */
static void print_buffer_to_window(WINDOW *win, int max_chars, int max_columns,
                                   char *buffer, int *curr_line) {
...
static void print_client_chat_buffer() {
  ...
  print_buffer_to_window(client_chat_window, MAX_CHARS_IN_CLIENT_TYPING_WINDOW,
                         /*	MAX_COLUMNS,	*/
...
char *get_client_name() { return client_user_name; }

/* Returns if were lurking. */
int get_is_lurking() { return is_lurking; }

/* Append text to the specified user window. */
void append_text_to_window(int num, char *text) {
  num--;
  if (num < 0 || num > 9) {
    log_writeln("Warning: Can't append text to window because uid/window = "
                "[%d] is not valid.",
                num);
    return;
  }
...
void clear_user_window_text(int num) {
  ...
  if (num < 0 || num > 9) {
    log_writeln(
        "Warning: Can't clear window because uid/window = [%d] is not valid.",
        num);
    return;
  }
...
char *grab_text_from_client_typing_window(void) { return client_buffer; }
...
void set_text_in_status_window(char *text) { strcpy(status_win_text, text); }
...
static char *write_to_ts_win(char *str, int max_col, int max_row, WINDOW *win,
                             char *buffer, int *line, int *buffer_size) {
...
void write_to_transcript_window(char *str) {
  ...
  for (i = 0; i <= length; i++) {
    ...
    if (str[i] == '\n' || str[i] == '\0') {
      ...
      //		f_transcript_buffer = write_to_ts_win(s,
      // TRANSCRIPT_MAX_COLUMNS*2, TRANSCRIPT_MAX_ROWS,
...
int main(int argc, char *argv[]) {
  ...
  //        set_text_in_status_window("asdkjf
  //        aklsjdf\n23874823974\niouqrweioruweir");
      ...
      */ // end get char
        ...
        /* Check if were in IM mode. */
          ...
          /*
           TODO: Display list of users (like deepsix) to send IM to.
                  All IM's will be displayed on the main transcript with some
           type of "marker"
        ...
        /* Check if were yelling. */
          ...
          switch (ch) {
...
#endif
              write_out(client_id); /*  enter key is pressed so send a message
                                       to the server. */
              break;
            ...
            {
              int i;
              for (i = client_cursor_position + 1; i < strlen(client_buffer);
                   i++) {
                client_buffer[i] = '\0';
              }
            } break;
            ...
            case 30: /* CTRL-6 */
              ...
              /*
               * If we encountered an unkown escape charcter, break out of here
               * so we don't
               * print it. */
              break; /* TODO: Fix me! */
            }
          ...
          case KEY_BACKSPACE:
            ...
            if (client_cursor_position == strlen(client_buffer)) {
               ...
               */
              int i;
              for (i = client_cursor_position - 2; i < strlen(client_buffer);
                   i++) {
          ...
          default:
            ...
            if (!iscntrl(ch) && (ch < 266 || ch > 266 + 19)) {
              /* Check if were inserting a character before the end of our
               * client
               * typing buffer. */
              if (client_cursor_position != strlen(client_buffer)) {
                ...
                int i;
                for (i = strlen(client_buffer) + 1; i > client_cursor_position;
                     i--) {
                  client_buffer[i] = client_buffer[i - 1];
                }
              }
