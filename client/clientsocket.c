  ...
  if (user_text[0] != '\0') {
    ...
    if (write(client_id, buffer, strlen(buffer)) == -1) {
      write_to_transcript_window(
          "Error: Couldn't write to server main chat message!\n");
      write_to_transcript_window(buffer);
    }
    /*
    if(write(client_id, status, strlen(status)*sizeof(char)) == -1) {
        write_to_transcript_window("Error: Couldn't write to server status
    message!\n");
       // write_to_transcript_window(buffer);
    }*/
...
void write_status(int client_id) {
  ...
  sprintf(update, "Total Sent: %d, Received: %d\nSeconds Connected: %ld | "
                  "Lurking: %s\nUser Name: %s",
          total_written, total_read, time(NULL) - time_connected, lurk_mode_tr,
          get_client_name());
  if (user_text[0] != '\0' && uid > 0) {
    ...
    if (written == -1) {
      write_to_transcript_window(
          "Error: Couldn't write to server user status message!\n");
    }
  }
...
void write_im(int client_id, int to_user) {
  ...
  if (written == -1) {
    write_to_transcript_window(
        "Error: Couldn't write to server for IM message!\n");
    // write_to_transcript_window(buffer);
  }
  free(message);
  clear_text_from_client_typing_window();
...
void write_vote(int client_id) {
  ...
  if (written == -1) {
    write_to_transcript_window(
        "Error: Couldn't write to server for vote message!\n");
    // write_to_transcript_window(buffer);
  }
  free(vote_message);
...
void write_lurk(int client_id) {
  ...
  if (written == -1) {
    write_to_transcript_window(
        "Error: Couldn't write to server that client is lurking!\n");
    // write_to_transcript_window(buffer);
  }
  free(lurk_result);
}
...
void read_from_server(int client_id) {
  ...
  if (bytes_read > 0) {
    ...
    case CMD_USERLIST: {
      ...
      // write_to_transcript_window(buf);
      switch (ul_type) {
      case USER_LIST_CURRENT: // This means we are getting the current user list
                              // from the server.
        ...
        do {
          ...
          user_list[user_num] = (UR_OBJ)malloc(sizeof(struct user_obj));
            ...
            (offset = get_next_user(offset, servout, user_list[user_num])) > 0);

        for (i = 0; i < user_num;
             i++) // adds users received to my online user list
        {
...
//                            set_window_user_name(i+1, temp_name);
        ...
        free(temp_user);
        // TODO: need seperate message for sign off in order to remove user from
        // online_list
        // also need a quick function to return index in online_list for window
        // number
        break;
      /* case USER_LIST_SIGN_OFF:
           ...
           while((offset = get_next_user(offset,servout,user_list[user_num])) >
       0);

           for(i=0; i<user_num; i++)           //adds users received to my
       online user list
           {
               online_user temp_o;
               temp_user = user_list[i];
